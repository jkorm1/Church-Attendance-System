...
<div class="container mx-auto py-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white p-4 rounded-lg shadow border">
            ...
            <div class="text-2xl font-bold text-green-600">{{ $attendance->filter(fn($present) => $present)->count() }}</div>
        </div>
        <div class="bg-white p-4 rounded-lg shadow border">
            <div class="text-sm text-gray-600">Absent</div>
            <div class="text-2xl font-bold text-red-600">{{ $attendance->filter(fn($present) => !$present)->count() }}</div>
        </div>
        <div class="bg-white p-4 rounded-lg shadow border">
            <div class="text-sm text-gray-600">Unmarked</div>
            <div class="text-2xl font-bold text-gray-500">{{ $members->count() - $attendance->count() }}</div>
        </div>
    </div>
