        ...
        <div class="flex min-h-screen">
            ...
            <aside class="sidebar w-64 p-8 flex flex-col gap-8 items-center justify-between hidden md:flex">
                <div class="flex flex-col gap-8 w-full">
                    <div class="flex flex-col items-center gap-2">
                        <img src="{{ asset('images/logoo.png') }}" alt="Church Logo" class="w-20 h-20 object-contain rounded-full border-4 border-[#f58502] bg-white" />
                        <span class="text-lg font-bold tracking-wide">Charisword Gospel Ministry</span>
                    </div>
                    <nav class="flex flex-col gap-3 mt-8 w-full">
                        <a href="{{ route('dashboard') }}" class="flex items-center gap-3 py-2 px-4 transition-all duration-150 @if(request()->routeIs('dashboard')) active-glow @endif">
                            <!-- Home Icon -->
                        ...
                        <a href="{{ route('members.index') }}" class="flex items-center gap-3 py-2 px-4 transition-all duration-150 @if(request()->routeIs('members.*')) active-glow @endif">
                            <!-- Users Icon -->
                        ...
                        <a href="{{ route('folds.index') }}" class="flex items-center gap-3 py-2 px-4 transition-all duration-150 @if(request()->routeIs('folds.*')) active-glow @endif">
                            <!-- Collection Icon -->
                        ...
                        <a href="{{ route('first_timers.index') }}" class="flex items-center gap-3 py-2 px-4 transition-all duration-150 @if(request()->routeIs('first_timers.*')) active-glow @endif">
                            <!-- User Add Icon -->
                        ...
                        <a href="{{ route('services.index') }}" class="flex items-center gap-3 py-2 px-4 transition-all duration-150 @if(request()->routeIs('services.*')) active-glow @endif">
                            <!-- Calendar Icon -->
                        ...
                        <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 py-2 px-4 transition-all duration-150 @if(request()->routeIs('profile.edit')) active-glow @endif">
                            <!-- User Circle Icon -->
                ...
                <div class="flex flex-col items-center gap-2">
                    <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name ?? 'User') }}&background=f58502&color=fff&size=128" alt="Profile" class="w-14 h-14 rounded-full border-2 border-[#f58502]" />
                    <span class="text-sm font-semibold">{{ auth()->user()->name ?? 'User' }}</span>
                </div>
            </aside>
            <!-- Main Content -->
            <main class="flex-1 flex flex-col min-h-screen main-content md:ml-64">
                ...
                <div class="relative mb-8">
                    <img src="{{ asset('images/logo.png') }}" alt="Ministry Hero" class="hero-img" />
