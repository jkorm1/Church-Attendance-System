    ...
    <body class="min-h-screen flex flex-col">
            ...
            <aside class="sidebar w-72 p-8 flex flex-col gap-8 items-center justify-between hidden lg:flex">
                <div class="flex flex-col gap-8 w-full">
                    ...
                    <div class="flex flex-col items-center gap-4">
                        <div class="relative">
                            <img src="{{ asset('images/logoo.png') }}" alt="Church Logo" class="w-24 h-24 object-contain rounded-full border-4 border-[#f58502] bg-white shadow-lg" />
                        </div>
                    ...
                    <nav class="sidebar-nav flex flex-col gap-3 mt-8 w-full">
                        <a href="{{ route('dashboard') }}" class="flex items-center gap-4 py-3 px-4 transition-all duration-200 @if(request()->routeIs('dashboard')) active-glow @endif">
                            <i class="fas fa-home text-lg"></i>
                            <span class="font-semibold">Dashboard</span>
                        </a>
                        <a href="{{ route('members.index') }}" class="flex items-center gap-4 py-3 px-4 transition-all duration-200 @if(request()->routeIs('members.*')) active-glow @endif">
                            <i class="fas fa-users text-lg"></i>
                            <span class="font-semibold">Members</span>
                        </a>
                        <a href="{{ route('folds.index') }}" class="flex items-center gap-4 py-3 px-4 transition-all duration-200 @if(request()->routeIs('folds.*')) active-glow @endif">
                            <i class="fas fa-layer-group text-lg"></i>
                            <span class="font-semibold">Folds</span>
                        </a>
                        <a href="{{ route('first_timers.index') }}" class="flex items-center gap-4 py-3 px-4 transition-all duration-200 @if(request()->routeIs('first_timers.*')) active-glow @endif">
                            <i class="fas fa-user-plus text-lg"></i>
                            <span class="font-semibold">First Timers</span>
                        </a>
                        <a href="{{ route('services.index') }}" class="flex items-center gap-4 py-3 px-4 transition-all duration-200 @if(request()->routeIs('services.*')) active-glow @endif">
                ...
                <div class="flex flex-col items-center gap-3">
                    <div class="relative">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name ?? 'User') }}&background=f58502&color=fff&size=128&font-size=0.4" alt="Profile" class="w-16 h-16 rounded-full border-3 border-[#f58502] shadow-lg" />
        ...
        <div id="mobile-nav" class="lg:hidden fixed inset-0 bg-black bg-opacity-50 z-40 hidden">
            <div class="absolute right-0 top-0 h-full w-80 bg-[#3a1d09] p-6 transform translate-x-full transition-transform duration-300">
                ...
                <nav class="space-y-4">
                    <a href="{{ route('dashboard') }}" class="flex items-center gap-4 py-3 px-4 text-white hover:bg-[#f58502] rounded-lg transition-all duration-200">
                        <i class="fas fa-home"></i>
                        <span>Dashboard</span>
                    </a>
                    <a href="{{ route('members.index') }}" class="flex items-center gap-4 py-3 px-4 text-white hover:bg-[#f58502] rounded-lg transition-all duration-200">
                        <i class="fas fa-users"></i>
                        <span>Members</span>
                    </a>
                    <a href="{{ route('folds.index') }}" class="flex items-center gap-4 py-3 px-4 text-white hover:bg-[#f58502] rounded-lg transition-all duration-200">
                        <i class="fas fa-layer-group"></i>
                        <span>Folds</span>
                    </a>
                    <a href="{{ route('first_timers.index') }}" class="flex items-center gap-4 py-3 px-4 text-white hover:bg-[#f58502] rounded-lg transition-all duration-200">
